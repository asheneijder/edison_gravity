...
class SwiftParserUtil
{
    ...
    public static function getMessageType(string $finContent): ?string
    {
        ...
        // [FIX] Update regex to allow '1' (common typo for 'I')
        if ($block2 && preg_match('/^[IO1]([0-9]{3})/', $block2, $matches)) {
            return $matches[1];
        }
        return null;
    }
    ...
    {
        $block2 = self::getBlock($finContent, '2');
        // [FIX] Allow '1' in receiver detection too
        if ($block2 && preg_match('/^[IO1][0-9]{3}[0-9]{4}[0-9]{6}([A-Z0-9]{8,11})/', $block2, $matches)) {
            return $matches[1];
        }
        if ($block2 && preg_match('/^[IO1][0-9]{3}.*?([A-Z0-9]{8,11})/', $block2, $matches)) {
             return $matches[1];
        }
        return null;
    }
    ...
    {
        $block2 = self::getBlock($finContent, '2');
        // [FIX] Allow '1' here as well
        if ($block2 && preg_match('/^[IO1][0-9]{3}[0-9]{4}([0-9]{6})/', $block2, $matches)) {
            return $matches[1];
        }
        return null;
    }
